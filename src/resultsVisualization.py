...
if args.showTestSetResults == '1':
    ax[0].set_title('PRC, valAUPRC={:.4f}, testAUPRC={:.4f}:'.format(valAP, testAP))
else:
    ax[0].set_title('PRC, valAUPRC={:.4f}:'.format(valAP))
...
if args.showTestSetResults == '1':
    ax[1].set_title('ROC, valAUROC={:.4f}, testAUROC={:.4f}'.format(valAUCROC, testAUCROC))
else:
    ax[1].set_title('ROC, valAUROC={:.4f}:'.format(valAUCROC))
ax[1].set_xlabel('FPR')
ax[1].set_ylabel('TPR')
ax[1].grid()
...
if args.showTestSetResults == '1':
    print("Test AUPRC: {:.2f}".format(testAP))
...
if args.negClassDominance != 0:
    negRatio = float(args.negClassDominance)
    ...
    newSoftValPreds = np.zeros((1, int(N.allNeg + N.sampledPos)), dtype=np.float32)
    newTargets = -1 * np.ones((1, int(N.allNeg + N.sampledPos)), dtype=np.float32)
    ...
    ax[0].set_ylim(-0.01, 1.01)
    ax[0].set_xlim(-0.01, 1.01)
    ax[0].set_title('PRC, valBalAUPRC={:.4f}, valImbalAUPRC={:.4f}:'.format(valAP, newAP))
    ax[0].set_xlabel('Recall')
    ax[0].set_ylabel('Precision')
    ax[0].grid()
    ax[0].legend(loc=0)
    ...
    ax[1].set_ylim(-0.01, 1.01)
    ax[1].set_xlim(-0.01, 1.01)
    ax[1].set_title('ROC, valBalAUROC={:.4f}, valImbalAUROC={:.4f}'.format(valAUCROC, newAUCROC))
    ax[1].set_xlabel('FPR')
    ax[1].set_ylabel('TPR')
    ax[1].grid()
